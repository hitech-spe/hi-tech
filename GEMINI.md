...
* Poiché si tratta di un'applicazione Angular Single Page App (SPA) configurata per l'SSR/Prerendering, Angular compila il fallback client-side (CSR) come `index.csr.html` nella cartella di distribuzione, mentre `index.html` viene generato per i singoli percorsi pre-renderizzati (es. `/home/index.html`). Per evitare errori 404 durante l'indicizzazione dei percorsi profondi (deep routing come `/services` o `/about`), è configurato un file `_redirects` (`/* /index.csr.html 200`) e la configurazione `netlify.toml` corrispondente. Questo istruisce la CDN (es. Netlify) a servire sempre `index.csr.html` con codice di stato 200 per qualsiasi rotta dinamica o non pre-renderizzata, lasciando la gestione del routing interno interamente ad Angular.
